export const itineraries = {
  varanasi: {
    title: "Varanasi 1-Day Yatra Package",
    slug: "varanasi",
    imageUrl: "/images/itinerary/Varanasi_2.jpg",
    price: "₹3,999",
    description:
      "Experience the divine aura of Kashi Vishwanath and the mesmerizing Ganga Aarti.",
    whyChooseThisYatra: [
      "✅ Comfortable travel with an experienced driver for a hassle-free journey.",
      "✅ Special care for elderly devotees, ensuring a smooth darshan experience.",
      "✅ Itinerary can be customized to suit your needs—just let us know!",
      "✅ Pay on arrival for a stress-free booking experience",
      "✅ Witness the divine Ganga Aarti at the sacred Dashashwamedh Ghat.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "🚗 Comfortable AC travel with an experienced driver for a hassle-free journey",
      "👵 Special care for elderly devotees, ensuring a smooth darshan experience",
      "🔄 Customizable itinerary—your yatra, your way!",
      "💰 Pay on arrival for a stress-free booking experience",
      "🌅 Witness the divine Ganga Aarti at the sacred Dashashwamedh Ghat",
    ],
    itinerary: [
      {
        day: "Day 1",
        activity:
          "🚗 Departure from Prayagraj – Early morning departure for a peaceful and hassle-free darshan.",
      },
      {
        day: "Day 1",
        activity:
          "🛥️ Kashi Vishwanath Temple Darshan – Seek blessings at one of the 12 Jyotirlingas.",
      },
      {
        day: "Day 1",
        activity:
          "🌸 Annapurna Devi Temple Visit – Offer prayers to Goddess Annapurna.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Vishalakshi Temple – A revered Shakti Peeth dedicated to Goddess Vishalakshi.",
      },
      {
        day: "Day 1",
        activity:
          "⚔️ Kal Bhairav Temple Visit – Pay homage to Lord Kal Bhairav, the guardian deity of Varanasi.",
      },
      {
        day: "Day 1",
        activity:
          "🍽️ Breakfast Break (Self-Paid) – Stop at a recommended restaurant.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Boat Ride at Dashashwamedh Ghat – Experience the mystical beauty of the Ganges.",
      },
      {
        day: "Day 1",
        activity:
          "🐵 Sankat Mochan Hanuman Temple Visit – Seek blessings from Lord Hanuman.",
      },
      {
        day: "Day 1",
        activity:
          "🍛 Lunch Break (Self-Paid) – Enjoy a traditional vegetarian meal.",
      },
      {
        day: "Day 1",
        activity: "🛒 Free Time for Shopping & Exploration (If time permits).",
      },
      {
        day: "Day 1",
        activity:
          "🌅 Ganga Aarti at Dashashwamedh Ghat – Witness the spectacular evening aarti.",
      },
      {
        day: "Day 1",
        activity:
          "🚗 Departure to Prayagraj – Drive back home with divine blessings.",
      },
    ],
    notIncluded: [
      "❌ Pickup & drop services (Available on request)",
      "❌ Breakfast & lunch (Enjoy self-paid meals at our recommended restaurants)",
      "❌ Personal expenses & shopping",
    ],
  },

  ayodhya: {
    title: "Ayodhya 1-Day Yatra",
    slug: "ayodhya",
    imageUrl: "/images/itinerary/Ayodhya_2.jpg",
    price: "₹4,499",
    description:
      "A soulful journey to the birthplace of Lord Ram, filled with devotion and history.",
    whyChooseThisYatra: [
      "✅ Flexible & customizable itinerary – Want to add something special? Just let us know!",
      "✅ Comfortable & stress-free travel – Just sit back, relax, and soak in the divine vibes!",
      "✅ Special care for elderly devotees – We ensure they travel with ease and comfort",
      "✅ Hassle-free darshan – Say goodbye to long queues and experience a smooth darshan.",
      "✅ No advance payment needed – Pay on arrival and book with confidence",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "🚗 Comfortable & stress-free travel – Just sit back, relax, and enjoy the divine vibes!",
      "👵 Special care for elderly devotees – Ensuring ease and comfort during the journey.",
      "🔄 Flexible & customizable itinerary – Want to add something special? Let us know!",
      "🛒 Hassle-free darshan – Skip long queues and experience a smooth darshan.",
      "💰 No advance payment – Pay on arrival and book with confidence.",
    ],
    itinerary: [
      {
        day: "Day 1",
        activity:
          "🚗 Departure from Prayagraj – Early morning journey to maximize your spiritual experience.",
      },
      {
        day: "Day 1",
        activity:
          "🚪 Sacred Snan at Ram Ki Paidi – Take a holy dip in the Sarayu River to cleanse your soul.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Hanuman Garhi Darshan – Seek blessings from Lord Hanuman before visiting Ram Janmabhoomi.",
      },
      {
        day: "Day 1",
        activity:
          "🏠 Ram Janmabhoomi Temple – Feel the divine presence at the birthplace of Lord Ram.",
      },
      {
        day: "Day 1",
        activity:
          "🍽️ Breakfast Break (Self-Paid) – Enjoy a delicious meal at a local restaurant.",
      },
      {
        day: "Day 1",
        activity:
          "🏠 Kanak Bhavan Visit – A stunning temple gifted to Goddess Sita as a wedding present.",
      },
      {
        day: "Day 1",
        activity:
          "🏛️ Dashrath Mahal – Explore the royal palace of King Dashrath, where Lord Ram spent his childhood.",
      },
      {
        day: "Day 1",
        activity:
          "🍛 Lunch Break (Self-Paid) – Savor a traditional vegetarian meal at a recommended eatery.",
      },
      {
        day: "Day 1",
        activity: "🛒 Free Time for Shopping & Exploration (If time permits).",
      },
      {
        day: "Day 1",
        activity:
          "🌇 Sarayu Aarti in the Evening – Witness the peaceful evening aarti by the Sarayu River.",
      },
      {
        day: "Day 1",
        activity:
          "🚗 Return to Prayagraj – Head back home carrying divine blessings.",
      },
    ],
    notIncluded: [
      "❌ Pickup & drop services (Available on request)",
      "❌ Breakfast & lunch (Enjoy self-paid meals at our recommended restaurants)",
      "❌ Personal expenses & shopping",
    ],
  },

  vindhyachal: {
    title: "1-Day vindhyachal Yatra from Prayagraj",
    slug: "vindhyachal",
    imageUrl: "/images/itinerary/Vindhyachal_2.jpg",
    price: "₹3,999",
    description:
      "A spiritual journey to the sacred temples of vindhyachal, with complete travel assistance.",
    whyChooseThisYatra: [
      "✅ Comfortable & Hassle-Free Travel",
      "✅ Flexible Itinerary (Customizable as per your preference)",
      "✅ Travel Assistance for a Smooth Experience.",
      "✅ Trusted & Well-Planned Pilgrimage.",
      "✅ Personalized Spiritual Experience – Optional special poojas available.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "🚗 Comfortable & Hassle-Free Travel",
      "🔄 Flexible Itinerary (Customizable as per your preference)",
      "🛒 Travel Assistance for a Smooth Experience",
      "💰 No advance payment – Pay on arrival and book with confidence.",
    ],
    itinerary: [
      { day: "Day 1", activity: "🚗 Afternoon Departure from Prayagraj" },
      { day: "Day 1", activity: "🙏 Kali Khoh Temple Darshan" },
      { day: "Day 1", activity: "🚶 Visit to Maa Ashtabhuja Temple" },
      { day: "Day 1", activity: "🛕 Darshan at Vindhyavasini Mata Temple" },
      { day: "Day 1", activity: "🍽️ Dinner Stop (Self-Paid)" },
      { day: "Day 1", activity: "🚘 Return to Prayagraj (Late Night Arrival)" },
    ],
    notIncluded: [
      "❌ No Accommodation Provided",
      "❌ Meals Not Included (Self-Paid Restaurant Stop)",
    ],
  },
  prayagraj: {
    title: "Prayagraj 1-Day Yatra",
    slug: "prayagraj",
    imageUrl: "/images/itinerary/Prayagraj_2.jpg",
    price: "₹3,999",
    description:
      "A soul-soothing spiritual journey with comfortable travel, divine darshans, and a touch of local culture!",
    whyChooseThisYatra: [
      "✅ Comfortable Stay Included – Relax in a well-rated hotel for a hassle-free experience.",
      "✅ Seamless Pickup & Drop-off – From Prayagraj Airport/Railway Station for a stress-free journey.",
      "✅ Sacred Triveni Snan & Boat Ride – Take a holy dip at the Sangam, the sacred confluence of three rivers.",
      "✅ AC Travel with an Experienced Driver – Enjoy a smooth and comfortable ride",
      "✅ Special Assistance for Elderly Devotees – We ensure a smooth and easy darshan experience.",
      "✅ Flexible Itinerary – Free time for shopping and exploring at your own pace",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      " Comfortable Stay Included – Relax in a well-rated hotel for a hassle-free experience.",
      " Seamless Pickup & Drop-off – From Prayagraj Airport/Railway Station for a stress-free journey.",
      " Sacred Triveni Snan & Boat Ride – Take a holy dip at the Sangam, the sacred confluence of three rivers.",
      " AC Travel with an Experienced Driver – Enjoy a smooth and comfortable ride.",
      " Special Assistance for Elderly Devotees – We ensure a smooth and easy darshan experience.",
      " Flexible Itinerary – Free time for shopping and exploring at your own pace.",
      " Pay on Arrival – No advance payment required, book with confidence!",
    ],
    itinerary: [
      {
        day: "Day 1",
        activity:
          "🚗 Pickup from Prayagraj Airport/Railway Station – Our driver will greet you upon arrival and take you to your hotel for a refreshing start.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Check-in & Breakfast at the Hotel – Freshen up and enjoy a complimentary breakfast before heading out for the divine journey.",
      },
      {
        day: "Day 1",
        activity:
          "🕉️ Triveni Snan & Boat Ride at Sangam – Experience the sacred confluence of the Ganga, Yamuna, and Saraswati rivers.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Seek blessings at this one-of-a-kind temple, home to a reclining idol of Lord Hanuman.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – A magnificent South Indian-style temple dedicated to Lord Shiva.",
      },
      {
        day: "Day 1",
        activity:
          "🍛 Lunch & Midday Rest at Hotel – Enjoy a delicious vegetarian meal (Self-Paid) and rest before continuing your exploration.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Alopi Devi Shakti Peeth – Visit this sacred Shakti Peeth, where it is believed that the last part of Goddess Sati fell.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Nag Vasuki Temple – Seek blessings at this temple dedicated to Lord Vasuki (King of Serpents).",
      },
      {
        day: "Day 1",
        activity:
          "🌅 Evening at Saraswati Ghat – Experience serenity by the river as you witness a breathtaking sunset over the Yamuna.",
      },
      {
        day: "Day 1",
        activity:
          "🛍️ Free Time for Shopping & Local Exploration – Stroll through the local markets for spiritual souvenirs.",
      },
      {
        day: "Day 1",
        activity:
          "🍽️ Dinner at a Partner Restaurant (Self-Paid) – End your day with a delicious meal of your choice.",
      },
      {
        day: "Day 1",
        activity:
          "🚗 Drop-off at Prayagraj Airport/Railway Station – After a fulfilling spiritual journey, we’ll ensure a comfortable ride back.",
      },
    ],
    notIncluded: [
      " Lunch & Dinner (Self-Paid) – You can order as per your taste at recommended restaurants.",
      " Entry fees or special pooja charges (if applicable) at temples.",
      " Personal expenses like shopping or additional sightseeing.",
      " Anything not explicitly mentioned in the inclusions.",
    ],
  },
  "prayagraj-varanasi": {
    title: "2-Day Prayagraj & Varanasi Yatra",
    slug: "prayagraj-varanasi",
    imageUrl: "/images/itinerary/Prayagraj-Varanasi_.jpg",
    price: "₹7,499",
    description:
      "A spiritually enriching 2-day yatra covering the sacred sites of Prayagraj & Varanasi with seamless travel arrangements.",
    whyChooseThisYatra: [
      "✅ Comfortable Stay & Smooth Transfers – We ensure a relaxing experience from arrival to departure.",
      "✅ Holy Dip & Darshan at Sacred Temples – Experience the divine aura of Triveni Sangam & Kashi Vishwanath Jyotirlinga.",
      "✅ Well-Planned Itinerary – No rush, just a peaceful & fulfilling yatra.",
      "✅ Elderly-Friendly Arrangements – Special attention for senior citizens.",
      "✅ Flexible & Customizable – We can tweak the plan to suit your needs.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "✅ Hassle-free spiritual experience with expert travel assistance",
      "✅ Comfortable hotel stay in Prayagraj with complimentary breakfast",
      "✅ Customizable itinerary to suit your needs",
      "✅ Optional pickup & drop-off service from Prayagraj (chargeable)",
    ],
    itinerary: [
      { day: "Day 1", activity: "🕘 Morning Arrival & Briefing" },
      {
        day: "Day 1",
        activity:
          "🚘 Pickup Assistance (Optional, Chargeable) – Pickup from anywhere in Prayagraj, complete booking formalities at branch office.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Hotel Check-in & Breakfast – Freshen up and enjoy a complimentary breakfast.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Triveni Sangam Snan & Boat Ride – Holy dip & optional pooja services available.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Seek blessings at the reclining Hanuman temple.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – Visit the beautiful South Indian-style temple.",
      },
      {
        day: "Day 1",
        activity: "🌆 Evening Exploration: Alopi Devi Temple & Shopping.",
      },
      {
        day: "Day 1",
        activity: "🍽️ Dinner at a recommended restaurant (Self-Paid).",
      },
      {
        day: "Day 1",
        activity: "🚘 Late-Night Departure for Varanasi (Overnight Journey).",
      },

      { day: "Day 2", activity: "🌅 Early Morning Arrival in Varanasi" },
      { day: "Day 2", activity: "🛕 Kashi Vishwanath Temple Darshan." },
      { day: "Day 2", activity: "🌸 Visit Annapurna Devi Temple." },
      { day: "Day 2", activity: "🔱 Visit Vishalakshi Temple." },
      { day: "Day 2", activity: "⚔️ Kal Bhairav Mandir Darshan." },
      {
        day: "Day 2",
        activity: "🍛 Breakfast at a recommended restaurant (Self-Paid).",
      },
      { day: "Day 2", activity: "🚣 Boat Ride on the Ganges." },
      { day: "Day 2", activity: "🌳 Visit Sankat Mochan Hanuman Temple." },
      { day: "Day 2", activity: "🛍️ Free Time for Shopping / Sightseeing." },
      {
        day: "Day 2",
        activity: "🌅 Witness Ganga Aarti at Dashashwamedh Ghat.",
      },
      {
        day: "Day 2",
        activity: "🚘 Return Journey to Prayagraj (Late Night Arrival).",
      },
    ],
    notIncluded: [
      "❌ No Accommodation in Varanasi (Only hotel stay in Prayagraj included)",
      "❌ Meals Not Included (We take you to well-rated restaurants)",
      "✅ Flexible Itinerary – Customizable as per customer preference",
    ],
  },
  "prayagraj-ayodhya": {
    title: "2-Day Prayagraj & Ayodhya Yatra",
    slug: "prayagraj-ayodhya",
    imageUrl: "/images/itinerary/Prayagraj-Ayodhya_2.jpg",
    price: "₹6,999",
    description:
      "A divine pilgrimage covering the sacred sites of Prayagraj & Ayodhya with seamless travel arrangements.",
    whyChooseThisYatra: [
      "✅ Hotel Accommodation Included – Comfortable stay at a well-rated hotel.",
      "✅ Triveni Snan & Boat Ride – Sacred boat ride to Sangam followed by a holy dip.",
      "✅ Comfortable AC Travel & Experienced Driver – Hassle-free journey throughout the trip.",
      "✅ Hassle-Free Darshans – With special care for families and senior citizens.",
      "✅ Customization Available – Personalize your yatra as per your spiritual and travel preferences",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "✅ Hassle-free spiritual experience with expert travel assistance",
      "✅ Comfortable hotel stay in Prayagraj with complimentary breakfast",
      "✅ Customizable itinerary to suit your needs",
      "✅ Optional pickup & drop-off service from Prayagraj (chargeable)",
    ],
    itinerary: [
      { day: "Day 1", activity: "🕘 Morning Arrival & Briefing" },
      {
        day: "Day 1",
        activity:
          "🚘 Pickup Assistance (Optional, Chargeable) – Pickup from anywhere in Prayagraj, complete booking formalities at branch office.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Hotel Check-in & Breakfast – Freshen up and enjoy a complimentary breakfast.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Triveni Sangam Snan & Boat Ride – Holy dip & optional pooja services available.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Seek blessings at the reclining Hanuman temple.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – Visit the beautiful South Indian-style temple.",
      },
      {
        day: "Day 1",
        activity: "🌆 Evening Exploration: Alopi Devi Temple & Shopping.",
      },
      {
        day: "Day 1",
        activity: "🍽️ Dinner at a recommended restaurant (Self-Paid).",
      },
      {
        day: "Day 1",
        activity: "🚘 Late-Night Departure for Ayodhya (Overnight Journey).",
      },

      { day: "Day 2", activity: "🌅 Early Morning Arrival in Ayodhya" },
      { day: "Day 2", activity: "🚿 Sacred Sarayu Snan." },
      {
        day: "Day 2",
        activity: "🍛 Breakfast at a recommended restaurant (Self-Paid).",
      },
      { day: "Day 2", activity: "🙏 Hanuman Garhi Temple Darshan." },
      { day: "Day 2", activity: "🛕 Shri Ram Janmabhoomi Temple Darshan." },
      {
        day: "Day 2",
        activity: "🍽️ Lunch Break at a recommended restaurant (Self-Paid).",
      },
      { day: "Day 2", activity: "🛕 Kanak Bhavan Temple Visit." },
      { day: "Day 2", activity: "🛕 Dasarath Mahal Visit." },
      { day: "Day 2", activity: "🌅 Saryu Ghat & Evening Aarti." },
      {
        day: "Day 2",
        activity: "🚘 Return Journey to Prayagraj (Late Night Arrival).",
      },
    ],
    notIncluded: [
      "❌ No Accommodation in Ayodhya (Only hotel stay in Prayagraj included)",
      "❌ Meals Not Included (We take you to well-rated restaurants)",
      "✅ Flexible Itinerary – Customizable as per customer preference",
    ],
  },
  "prayagraj-vindhyachal": {
    title: "2-Day Prayagraj & Vindhyanchal Yatra",
    slug: "prayagraj-vindhyachal",
    imageUrl: "/images/itinerary/Prayagraj-Vindhyachal_2.jpg",
    price: "₹6,999",
    description:
      "An enriching spiritual journey covering the divine sites of Prayagraj & Vindhyanchal with seamless travel support.",
    whyChooseThisYatra: [
      "✅ Optimized Route – No backtracking, smooth transitions between destinations.",
      "✅ Comfortable Travel – AC Sedan (5-seater), SUV (7-seater), Traveller, or AC Bus based on group size.",
      "✅ Flexible & Hassle-Free – Customizable itinerary, senior citizen-friendly, pay-on-arrival option.",
      "✅ Handpicked Stays – Well-rated hotels with breakfast.",
      "✅ Personalized Spiritual Experience – Optional special poojas available.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "✅ Hassle-free spiritual experience with expert travel assistance.",
      "✅ Comfortable hotel stay in Prayagraj with complimentary breakfast",
      "✅ Customizable itinerary to suit your needs",
      "✅ Optional pickup & drop-off service from Prayagraj (chargeable)",
    ],
    itinerary: [
      { day: "Day 1", activity: "🕘 Morning Arrival & Briefing" },
      {
        day: "Day 1",
        activity:
          "🚘 Pickup Assistance (Optional, Chargeable) – Pickup from anywhere in Prayagraj, complete booking formalities at branch office.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Hotel Check-in & Breakfast – Freshen up and enjoy a complimentary breakfast.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Triveni Sangam Snan & Boat Ride – Holy dip & optional pooja services available.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Seek blessings at the reclining Hanuman temple.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – Visit the beautiful South Indian-style temple.",
      },
      {
        day: "Day 1",
        activity: "🌆 Evening Exploration: Alopi Devi Temple & Shopping.",
      },
      {
        day: "Day 1",
        activity: "🍽️ Dinner at a recommended restaurant (Self-Paid).",
      },
      { day: "Day 1", activity: "🏨 Back to Hotel – Overnight Stay." },

      { day: "Day 2", activity: "🌅 Breakfast at Hotel & Checkout" },
      {
        day: "Day 2",
        activity: "🙏 Nag Vasuki Temple & Mankameshwar Temple Darshan.",
      },
      {
        day: "Day 2",
        activity: "🍽️ Lunch Break at a recommended restaurant (Self-Paid).",
      },
      { day: "Day 2", activity: "🚘 Afternoon: Journey to Vindhyanchal." },
      { day: "Day 2", activity: "🛕 Kali Khoh Temple Darshan." },
      { day: "Day 2", activity: "🚶 Visit to Maa Ashtabhuja Temple." },
      { day: "Day 2", activity: "🛕 Vindhyavasini Mata Darshan." },
      {
        day: "Day 2",
        activity: "🍽️ Dinner at a recommended restaurant (Self-Paid).",
      },
      {
        day: "Day 2",
        activity: "🚘 Return to Prayagraj (Late Night Arrival).",
      },
    ],
    notIncluded: [
      "❌ No Accommodation in Vindhyanchal (Only hotel stay in Prayagraj included)",
      "❌ Meals Not Included (We take you to well-rated restaurants)",
      "✅ Flexible Itinerary – Customizable as per customer preference",
    ],
  },
  "prayagraj-ayodhya-varanasi": {
    title: "Prayagraj, Ayodhya, and Varanasi 3-Day Yatra",
    slug: "prayagraj-ayodhya-varanasi",
    imageUrl: "/images/itinerary/Prayagraj-Varanasi-Ayodhya_2.jpg",
    price: "₹12,999",
    description:
      "A soulful journey through India’s most sacred cities, designed with care, comfort, and flexibility to meet your every need.",
    whyChooseThisYatra: [
      "✅ Comfort & Convenience: Enjoy well-rated hotel stays with complimentary breakfast at all locations.",
      "✅ Travel Booking Assistance - Available From Anywhere in India.",
      "✅ Seamless Travel: Comfortable AC travel with an experienced driver who knows the pilgrimage routes.",
      "✅ Personalized Experience: Our itinerary is flexible—customize your yatra as per your preferences.      .",
      "✅ Special Care: We take extra care of families and elderly devotees, ensuring a stress-free journey.      .",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "🚗 Comfortable AC travel with an experienced driver who knows the pilgrimage routes",
      "🛏️ Well-rated hotel stays with complimentary breakfast at all locations",
      "👵 Special care for families and elderly devotees for a stress-free journey",
      "🔄 Customizable itinerary – Personalize your experience!",
      "💰 Pay on arrival – No advance payment required",
      "✈️ Travel booking assistance available from anywhere in India",
      "✅ Get the best train/flight options absolutely free!",
      "🛎️ Want us to handle your travel? Let’s plan it together!",
    ],
    itinerary: [
      { day: "Day 1", activity: "🕘 Morning Arrival & Briefing" },
      {
        day: "Day 1",
        activity:
          "🚘 Pickup Assistance (Optional, Chargeable) – Pickup from Prayagraj Airport/Railway Station and transfer to the branch office for briefing and customization options.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Hotel Check-in & Breakfast – Freshen up and enjoy a complimentary breakfast.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Triveni Sangam Snan & Boat Ride – Holy dip at the confluence of Ganga, Yamuna, and Saraswati with an included boat ride.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Visit this unique temple known for its reclining idol of Lord Hanuman.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – Explore this beautifully crafted South Indian-style temple.",
      },
      {
        day: "Day 1",
        activity:
          "🍽️ Lunch Break (Self-Paid) – Enjoy a vegetarian meal at a recommended restaurant.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Alopi Devi Shakti Peeth Darshan – Visit this sacred Shakti Peeth.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Nag Vasuki Temple Visit – Seek blessings at the temple dedicated to Lord Vasuki.",
      },
      { day: "Day 1", activity: "🛍️ Free Time for Shopping & Exploration." },
      {
        day: "Day 1",
        activity: "🍽️ Dinner (Self-Paid) at a recommended restaurant.",
      },
      {
        day: "Day 1",
        activity: "🚘 Departure for Ayodhya (Overnight Journey).",
      },

      { day: "Day 2", activity: "🌅 Early Morning Arrival in Ayodhya" },
      {
        day: "Day 2",
        activity: "🏨 Hotel Check-in & Complimentary Breakfast.",
      },
      {
        day: "Day 2",
        activity: "🚿 Sarayu Snan – Holy dip in the sacred Sarayu River.",
      },
      {
        day: "Day 2",
        activity:
          "🙏 Hanuman Garhi Darshan – Seek blessings at this iconic temple.",
      },
      {
        day: "Day 2",
        activity:
          "🙏 Shri Ram Janmabhoomi Darshan – Visit Lord Ram’s birthplace.",
      },
      {
        day: "Day 2",
        activity:
          "🍽️ Lunch Break (Self-Paid) – Authentic vegetarian meal at a trusted restaurant.",
      },
      {
        day: "Day 2",
        activity:
          "🛕 Kanak Bhavan Visit – Explore this beautiful temple gifted to Goddess Sita.",
      },
      {
        day: "Day 2",
        activity:
          "🏰 Dasharath Mahal Visit – Discover the historical palace of King Dasharath.",
      },
      {
        day: "Day 2",
        activity:
          "🕯️ Sarayu Aarti – Witness the mesmerizing evening aarti by the river.",
      },
      {
        day: "Day 2",
        activity: "🚘 Departure for Varanasi (Overnight Journey).",
      },

      {
        day: "Day 3",
        activity: "🌄 Morning Arrival in Varanasi & Hotel Check-in",
      },
      { day: "Day 3", activity: "🍽️ Complimentary Breakfast." },
      {
        day: "Day 3",
        activity:
          "🚿 Ganga Snan at Dashashwamedh Ghat – Holy dip in the sacred Ganga.",
      },
      {
        day: "Day 3",
        activity:
          "🙏 Kashi Vishwanath Jyotirlinga Darshan – Seek blessings at this revered Shiva shrine.",
      },
      {
        day: "Day 3",
        activity:
          "🙏 Annapurna Devi Temple Visit – Offer prayers to Goddess Annapurna.",
      },
      {
        day: "Day 3",
        activity:
          "🙏 Kal Bhairav Temple Visit – Pay homage to the fierce guardian deity of Kashi.",
      },
      {
        day: "Day 3",
        activity:
          "🚣 Boat Ride on the Ganges – Experience the ghats from a serene boat ride.",
      },
      {
        day: "Day 3",
        activity:
          "🍽️ Lunch Break (Self-Paid) – Traditional Banarasi meal at a recommended restaurant.",
      },
      {
        day: "Day 3",
        activity:
          "🙏 Sankat Mochan Hanuman Temple Darshan – Visit this powerful Hanuman temple.",
      },
      {
        day: "Day 3",
        activity:
          "🛍️ Free Time for Shopping – Explore Banarasi silk sarees, rudraksha malas, and souvenirs.",
      },
      {
        day: "Day 3",
        activity:
          "🕯️ Ganga Aarti at Dashashwamedh Ghat – Witness the breathtaking evening Aarti.",
      },
      {
        day: "Day 3",
        activity:
          "🚘 Drop-off: Choose between Varanasi Airport/Railway Station or return to Prayagraj (Chargeable).",
      },
    ],
    notIncluded: [
      "❌ Pickup & drop services (Available on request)",
      "❌ Meals (Lunch & Dinner – Self-paid at recommended restaurants)",
      "❌ Entry Fees or Special Pooja Charges (if applicable)",
      "❌ Personal expenses & shopping",
    ],
  },
  "prayagraj-vindhyachal-ayodhya": {
    title: "Prayagraj, Vindhyanchal, and Ayodhya 3-Day Yatra",
    slug: "prayagraj-vindhyachal-ayodhya",
    imageUrl: "/images/itinerary/Prayagraj-Vindhyachal-Ayodhya_2.jpg",
    price: "₹11,999",
    description:
      "A soulful journey through India’s sacred cities, designed with comfort, flexibility, and a deep spiritual experience.",
    whyChooseThisYatra: [
      "✅ Optimized Route – No backtracking, smooth transitions between destinations.",
      "✅ Comfortable Travel – AC Sedan (5-seater), SUV (7-seater), Traveller, or AC Bus based on group size.",
      "✅ Flexible & Hassle-Free – Customizable itinerary, senior citizen-friendly, pay-on-arrival option.",
      "✅ Handpicked Stays – Well-rated hotels with breakfast.",
      "✅ Personalized Spiritual Experience – Optional special poojas available.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "🚗 Comfortable AC travel with an experienced driver who knows the pilgrimage routes",
      "🛏️ Well-rated hotel stays with complimentary breakfast at all locations",
      "👵 Special care for families and elderly devotees for a stress-free journey",
      "🔄 Customizable itinerary – Personalize your experience!",
      "💰 Pay on arrival – No advance payment required",
      "✈️ Travel booking assistance available from anywhere in India",
      "✅ Get the best train/flight options absolutely free!",
      "🛎️ Want us to handle your travel? Let’s plan it together!",
    ],
    itinerary: [
      { day: "Day 1", activity: "🕘 Morning Arrival & Briefing" },
      {
        day: "Day 1",
        activity:
          "🚘 Pickup Assistance (Optional, Chargeable) – Pickup from Prayagraj and transfer to the branch office for briefing and customization options.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Hotel Check-in & Breakfast – Freshen up and enjoy a complimentary breakfast.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Triveni Sangam Snan & Boat Ride – Holy dip at the confluence of Ganga, Yamuna, and Saraswati with an included boat ride.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Visit this unique temple known for its reclining idol of Lord Hanuman.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – Explore this beautifully crafted South Indian-style temple.",
      },
      {
        day: "Day 1",
        activity:
          "🍽️ Lunch Break (Self-Paid) – Enjoy a vegetarian meal at a recommended restaurant.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Alopi Devi Shakti Peeth Darshan – Visit this sacred Shakti Peeth.",
      },
      { day: "Day 1", activity: "🛍️ Free Time for Shopping & Exploration." },
      {
        day: "Day 1",
        activity: "🍽️ Dinner (Self-Paid) at a recommended restaurant.",
      },
      { day: "Day 1", activity: "🚘 Overnight Stay in Prayagraj." },

      {
        day: "Day 2",
        activity: "🌅 Morning in Prayagraj & Departure for Vindhyanchal",
      },
      {
        day: "Day 2",
        activity: "🍽️ Complimentary Breakfast & Hotel Checkout.",
      },
      {
        day: "Day 2",
        activity: "🚘 Drive to Vindhyanchal – Arrive by afternoon.",
      },
      {
        day: "Day 2",
        activity:
          "🙏 Vindhyanchal Mata Mandir Darshan – Seek blessings at this revered temple.",
      },
      {
        day: "Day 2",
        activity:
          "🛕 Ashtabhuja Devi Darshan – Visit the temple of Goddess Ashtabhuja.",
      },
      {
        day: "Day 2",
        activity:
          "🛕 Kali Khoh Temple Visit – Explore this sacred cave temple.",
      },
      { day: "Day 2", activity: "🍽️ Dinner Break (Self-Paid)." },
      {
        day: "Day 2",
        activity:
          "🚘 Evening Departure for Ayodhya – Reach by late night and check-in to hotel.",
      },
      { day: "Day 2", activity: "🏨 Overnight Stay in Ayodhya." },

      { day: "Day 3", activity: "🌄 Ayodhya – The Abode of Divinity" },
      {
        day: "Day 3",
        activity: "🍽️ Complimentary Breakfast & Refreshing Start.",
      },
      {
        day: "Day 3",
        activity: "🚿 Sarayu Snan – Holy dip in the sacred Sarayu River.",
      },
      {
        day: "Day 3",
        activity:
          "🙏 Hanuman Garhi Darshan – Seek blessings at this iconic temple.",
      },
      {
        day: "Day 3",
        activity:
          "🙏 Shri Ram Janmabhoomi Darshan – Visit Lord Ram’s birthplace.",
      },
      {
        day: "Day 3",
        activity:
          "🍽️ Lunch Break (Self-Paid) – Authentic vegetarian meal at a trusted restaurant.",
      },
      {
        day: "Day 3",
        activity:
          "🛕 Kanak Bhavan Visit – Explore this beautiful temple gifted to Goddess Sita.",
      },
      {
        day: "Day 3",
        activity:
          "🏰 Dasharath Mahal Visit – Discover the historical palace of King Dasharath.",
      },
      {
        day: "Day 3",
        activity:
          "🕯️ Sarayu Aarti – Witness the mesmerizing evening aarti by the river.",
      },
      {
        day: "Day 3",
        activity:
          "🚘 Hotel Checkout & Departure – Return to Prayagraj or drop-off at Ayodhya Railway Station (Chargeable).",
      },
    ],
    notIncluded: [
      "❌ Pickup & drop services (Available on request)",
      "❌ Meals (Lunch & Dinner – Self-paid at recommended restaurants)",
      "❌ Entry Fees or Special Pooja Charges (if applicable)",
      "❌ Personal expenses & shopping",
    ],
  },
  "prayagraj-vindhyachal-varanasi": {
    title: "Prayagraj, Vindhyanchal, and Kashi Yatra (2 Nights / 3 Days)",
    slug: "prayagraj-vindhyachal-varanasi",
    imageUrl: "/images/itinerary/Prayagraj-Vindhyachal-Varanasi_2.jpg",
    price: "₹11,999",
    description:
      "A sacred pilgrimage designed for a smooth and enriching experience.",
    whyChooseThisYatra: [
      "✅ Comfortable AC travel & well-rated hotels.",
      "✅ Flexible itinerary—customize as per your preferences.",
      "✅ Special assistance for senior citizens & families.",
      "✅ Pay-on-arrival booking with hassle-free arrangements.",
      "✅ No rush—relaxed darshans & sightseeing.",
      "✅ Comfortable Travel – AC Sedan (5-seater), SUV (7-seater), Traveller, or AC Bus based on group size.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "🚗 Comfortable AC travel with an experienced driver who knows the pilgrimage routes",
      "🛏️ Well-rated hotel stays with complimentary breakfast at all locations",
      "👵 Special care for families and elderly devotees for a stress-free journey",
      "🔄 Customizable itinerary – Personalize your experience!",
      "💰 Pay on arrival – No advance payment required",
      "✈️ Travel booking assistance available from anywhere in India",
      "✅ Get the best train/flight options absolutely free!",
      "🛎️ Want us to handle your travel? Let’s plan it together!",
    ],
    itinerary: [
      { day: "Day 1", activity: "🕘 Morning Arrival & Briefing" },
      {
        day: "Day 1",
        activity:
          "🚘 Pickup Assistance (Optional, Chargeable) – Pickup from Prayagraj and transfer to the branch office for briefing and customization options.",
      },
      {
        day: "Day 1",
        activity:
          "🏨 Hotel Check-in & Breakfast – Freshen up and enjoy a complimentary breakfast.",
      },
      {
        day: "Day 1",
        activity:
          "🚣 Triveni Sangam Snan & Boat Ride – Holy dip at the confluence of Ganga, Yamuna, and Saraswati with an included boat ride.",
      },
      {
        day: "Day 1",
        activity:
          "🙏 Bade Hanuman Ji Temple Darshan – Visit this unique temple known for its reclining idol of Lord Hanuman.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Shankar Viman Mandapam – Explore this beautifully crafted South Indian-style temple.",
      },
      {
        day: "Day 1",
        activity:
          "🍽️ Lunch Break (Self-Paid) – Enjoy a vegetarian meal at a recommended restaurant.",
      },
      {
        day: "Day 1",
        activity:
          "🛕 Alopi Devi Shakti Peeth Darshan – Visit this sacred Shakti Peeth.",
      },
      { day: "Day 1", activity: "🛍️ Free Time for Shopping & Exploration." },
      {
        day: "Day 1",
        activity: "🍽️ Dinner (Self-Paid) at a recommended restaurant.",
      },
      { day: "Day 1", activity: "🚘 Overnight Stay in Prayagraj." },

      {
        day: "Day 2",
        activity: "🌅 Morning in Prayagraj & Departure for Vindhyanchal",
      },
      {
        day: "Day 2",
        activity: "🍽️ Complimentary Breakfast & Hotel Checkout.",
      },
      {
        day: "Day 2",
        activity: "🚘 Drive to Vindhyanchal – Arrive by afternoon.",
      },
      {
        day: "Day 2",
        activity:
          "🙏 Vindhyanchal Mata Mandir Darshan – Seek blessings at this revered temple.",
      },
      {
        day: "Day 2",
        activity: "🛍️ Optional Shopping Time for Religious Items & Prasad.",
      },
      { day: "Day 2", activity: "🍽️ Dinner Break (Self-Paid)." },
      {
        day: "Day 2",
        activity:
          "🚘 Evening Departure for Kashi (Varanasi) – Reach by late night and check-in to hotel.",
      },
      { day: "Day 2", activity: "🏨 Overnight Stay in Varanasi." },

      { day: "Day 3", activity: "🌄 Kashi – The Spiritual Capital" },
      {
        day: "Day 3",
        activity: "🌊 Ganga Snan at Assi Ghat or Dashashwamedh Ghat.",
      },
      {
        day: "Day 3",
        activity:
          "🕉️ Darshan at Kashi Vishwanath Temple (One of the 12 Jyotirlingas).",
      },
      {
        day: "Day 3",
        activity: "🙏 Annapurna Devi Temple Darshan (Goddess of nourishment).",
      },
      {
        day: "Day 3",
        activity: "🕉️ Kal Bhairav Temple Visit (Guardian deity of Kashi).",
      },
      {
        day: "Day 3",
        activity:
          "⛵ Scenic Boat Ride on the Ganga – Witness historic ghats & temples.",
      },
      { day: "Day 3", activity: "🍽️ Lunch Break (Self-Paid)." },
      {
        day: "Day 3",
        activity:
          "🌿 Sankat Mochan Hanuman Temple Visit – A revered Hanuman Ji temple.",
      },
      {
        day: "Day 3",
        activity:
          "🕯️ Ganga Aarti at Dashashwamedh Ghat – A mesmerizing spiritual experience.",
      },
      {
        day: "Day 3",
        activity:
          "🏨 Hotel Checkout & Departure – Drop-off at Prayagraj or Varanasi Railway Station (Chargeable).",
      },
    ],
    notIncluded: [
      "❌ Pickup & drop services (Available on request)",
      "❌ Meals (Lunch & Dinner – Self-paid at recommended restaurants)",
      "❌ Entry Fees or Special Pooja Charges (if applicable)",
      "❌ Personal expenses & shopping",
    ],
  },
  "prayagraj-vindhyachal-ayodhya-varanasi": {
    title: "Prayagraj – Vindhyanchal – Ayodhya – Varanasi (3 Nights / 4 Days)",
    slug: "prayagraj-vindhyachal-ayodhya-varanasi",
    imageUrl: "/images/itinerary/Prayagraj-Vindhyachal-Varanasi-Ayodhya_2.jpg",
    price: "₹15,999",
    description:
      "A seamless pilgrimage covering four sacred cities with spiritual depth, comfort, and flexibility.",
    whyChooseThisYatra: [
      "✅ Optimized Route – No backtracking, smooth transitions between destinations.",
      "✅ Comfortable Travel – AC Sedan (5-seater), SUV (7-seater), Traveller, or AC Bus based on group size.",
      "✅ Flexible & Hassle-Free – Customizable itinerary, senior citizen-friendly, pay-on-arrival option.",
      "✅ Handpicked Stays – Well-rated hotels with breakfast.",
      "✅ Personalized Spiritual Experience – Optional special poojas available.",
    ],
    routeOverview: [
      "🚗 Day 1: Prayagraj Arrival, Darshan & Stay.",
      "🚗 Day 2: Prayagraj → Vindhyanchal → Ayodhya (Night Stay).",
      "🚗 Day 3: Ayodhya → Varanasi (Night Stay).",
      "🚗 Day 4: Varanasi Darshan & Departure.",
    ],
    highlights: [
      "✅ Optimized Route – No backtracking, smooth transitions between destinations.",
      "✅ Comfortable Travel – AC Sedan, SUV, Traveller, or AC Bus based on group size.",
      "✅ Flexible & Hassle-Free – Customizable itinerary, senior citizen-friendly.",
      "✅ Handpicked Stays – Well-rated hotels with breakfast.",
      "✅ Personalized Spiritual Experience – Optional special poojas available.",
    ],
    itinerary: [
      { day: "Day 1", activity: "🚗 Arrival & Welcome in Prayagraj." },
      { day: "Day 1", activity: "🚣 Triveni Sangam Snan & Boat Ride." },
      {
        day: "Day 1",
        activity: "🛕 Bade Hanuman Ji & Shankar Viman Mandapam.",
      },
      { day: "Day 1", activity: "🛕 Alopi Devi Temple Visit & Shopping." },
      { day: "Day 2", activity: "🚗 Travel to Vindhyanchal & Darshan." },
      { day: "Day 2", activity: "🚗 Evening Travel to Ayodhya." },
      { day: "Day 2", activity: "🏨 Overnight Stay in Ayodhya." },
      {
        day: "Day 3",
        activity: "🌅 Holy Sarayu Snan & Hanuman Garhi Darshan.",
      },
      { day: "Day 3", activity: "🛕 Shri Ram Janmabhoomi & Kanak Bhavan." },
      { day: "Day 3", activity: "🚣 Optional Boat Ride on Sarayu River." },
      {
        day: "Day 3",
        activity: "🪔 Evening Sarayu Aarti & Travel to Varanasi.",
      },
      {
        day: "Day 4",
        activity: "🚿 Holy Ganga Snan & Kashi Vishwanath Darshan.",
      },
      {
        day: "Day 4",
        activity: "🚣 Scenic Ganga Boat Ride & Sankat Mochan Temple.",
      },
      { day: "Day 4", activity: "🕯️ Ganga Aarti at Dashashwamedh Ghat." },
      { day: "Day 4", activity: "🏁 Departure & Travel Assistance." },
    ],
    notIncluded: [
      "❌ Meals (Lunch & Dinner) – Self-paid at partner restaurants.",
      "❌ Entry fees, special pooja charges (if applicable).",
      "❌ Pickup & drop-off charges (if applicable).",
      "❌ Personal expenses (shopping, tips, donations, porter charges).",
    ],
  },
};
